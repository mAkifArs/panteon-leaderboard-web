...
export function Podium({ entries, selfUserId }: PodiumProps): React.ReactElement | null {
  if (entries.length < 3) return null
  const [first, second, third] = entries
  const isSelf = (entry: ViewEntry): boolean => entry.userId === selfUserId
    ...
    <section aria-label="Top 3 podium" className="relative mb-6 mt-6 px-1 md:mb-8 md:mt-8 md:px-6">
      ...
      <div className="relative grid grid-cols-2 gap-2.5 md:hidden">
        <div className="col-span-2">
          <PodiumCard entry={first} place={1} isSelf={isSelf(first)} />
        </div>
        <PodiumCard entry={second} place={2} isSelf={isSelf(second)} />
        <PodiumCard entry={third} place={3} isSelf={isSelf(third)} />
      </div>
